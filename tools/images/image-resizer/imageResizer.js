...
function initImageResizer() {
    ...
    if (previewWrapper) { previewWrapper.addEventListener('dragover', (e) => { e.preventDefault(); e.stopPropagation(); previewWrapper.classList.add('drag-over'); }); previewWrapper.addEventListener('dragleave', (e) => { e.preventDefault(); e.stopPropagation(); previewWrapper.classList.remove('drag-over'); }); previewWrapper.addEventListener('drop', (e) => { e.preventDefault(); e.stopPropagation(); previewWrapper.classList.remove('drag-over'); const files = e.dataTransfer.files; if (files.length > 0) { imageUpload.files = files; handleImageFile(files[0]); } }); if (imageUploadPlaceholder) imageUploadPlaceholder.addEventListener('click', () => imageUpload.click()); previewWrapper.addEventListener('click', () => { if (!currentImageSrc) imageUpload.click(); }); }
