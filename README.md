...
3.  **Regenerate SEO artifacts (optional):**
    `build.js` regenerates `sitemap.xml` (with real last-modified dates from git history) and refreshes the JSON-LD structured data (`WebApplication` / `BreadcrumbList` / `FAQPage`) plus `og:image` tags on every tool page:
    ```bash
    node build.js
    ```
    Schema blocks in tool pages are marked "generated by build.js" - edit `js/tools-data.js` or the page FAQs instead of hand-editing those blocks.
